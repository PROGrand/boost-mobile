...
: ${BOOST_LIBS:="chrono program_options system thread random regex"}
...
buildBoostForIPhoneOS()
{
    ...
    ./b2 -j16 linkflags="-stdlib=libc++" --build-dir=iphone-build --stagedir=iphone-build/stage --prefix=$PREFIXDIR --toolset=darwin architecture=arm target-os=iphone macosx-version=iphone-${IPHONE_SDKVERSION} define=_LITTLE_ENDIAN link=static stage
    ./b2 -j16 linkflags="-stdlib=libc++" --build-dir=iphone-build --stagedir=iphone-build/stage --prefix=$PREFIXDIR --toolset=darwin architecture=arm target-os=iphone macosx-version=iphone-${IPHONE_SDKVERSION} define=_LITTLE_ENDIAN link=static install
    doneSection

    ./b2 -j16 linkflags="-stdlib=libc++" --build-dir=iphonesim-build --stagedir=iphonesim-build/stage --toolset=darwin-${IPHONE_SDKVERSION}~iphonesim architecture=x86 target-os=iphone macosx-version=iphonesim-${IPHONE_SDKVERSION} link=static stage
    doneSection
